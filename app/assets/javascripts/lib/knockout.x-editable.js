				...
				init : function(element, valueAccessor, allBindingsAccessor,
					...
					if (!editableOptions.validate && value.isValid) {
						editableOptions.validate = function(testValue) {
							// have to set to new value, then call validate,
							// then reset to original value
							// not pretty, but works
							var initalValue = value();
							value(testValue);
							var res = value.isValid() ? null : ko.utils
									.unwrapObservable(value.error);
							value(initalValue);
							return res;
						}
					}

					if ((editableOptions.type === 'select'
						...
						function applyToObject(object, predicate, defaultValue) {
							...
							if (predicateType == "function") // Given a
								// function; run
								// it against
								// the data
								// value
								return predicate(object);
							else if (predicateType == "string") // Given a
								// string; treat
								// it as a
								// property name
								// on the data
								...
								// Given no optionsText arg; use the data value
								// itself
								return defaultValue;
						}
									...
									.arrayMap(
											...
											function(item) {
												var optionValue = applyToObject(
														item,
														editableOptions.optionsValue,
														item);
												var optionText = applyToObject(
														item,
														editableOptions.optionsText,
														optionText);
												...
												return {
													...
													text : ko.utils
															.unwrapObservable(optionText)
												};
											});
						}
					}

					if (editableOptions.visible
							&& ko.isObservable(editableOptions.visible)) {
					...
					// setup observable to fire only when editable changes, not
					// when options change
					...
					if (editableOptions.visible
						...
						ko.computed({
							read : function() {
								var val = ko.utils
										.unwrapObservable(editableOptions
												.visible());
								if (val)
